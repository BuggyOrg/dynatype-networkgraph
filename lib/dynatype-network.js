...
function addTypeConverting(processGraph, convertGraph) {
  var fs = require('fs');
  var componentLib = require('../component-library');
  ...
  for (var j = 0; j < processGraph.edgeCount(); j++) {
    ...
    if ((processGraph.node(edges[j].v)['nodeType'] === 'inPort' || processGraph.node(edges[j].v)['nodeType'] === 'outPort') && (processGraph.node(edges[j].w)['nodeType'] === 'inPort' || processGraph.node(edges[j].w)['nodeType'] === 'outPort')) {
      ...
      console.log(metaV);
      console.log(compLib[metaV]);
      var typeV;
      var typeW;
      if (processGraph.node(edges[j].v)['nodeType'] === 'inPort') {
        typeV = compLib[metaV]['inputPorts'][portNameV];
      } else {
        typeV = compLib[metaV]['outputPorts'][portNameV];
      }
      if (processGraph.node(edges[j].w)['nodeType'] === 'inPort') {
        typeW = compLib[metaW]['inputPorts'][portNameW];
      } else {
        typeW = compLib[metaW]['outputPorts'][portNameW];
      }
